...
The central phospho-acceptor site is located at position `0` and only measures the relative favorability of serine over threonine. The user can exclude this favorability measure by setting the parameter `includeSTfavorability` to `FALSE`, in which case all central log2-odd scores are set to `NA`.
...
If a peptide contains several phosphorylated residues, option `onlyCentralAcceptor` controls how to select the acceptor position. Setting `onlyCentralAcceptor` to `FALSE` will return all possible aligned phosphosites for a given input peptide. Note that in this case the output is not parallel to the input.
...
```{r}
scores <- scorePhosphosites(pwms, sites$processed)
...
By default, the resulting score is the log2-odds score of the PWM. Alternatively, by setting `scoreType` to `percentile`, a percentile rank of the log2-odds score is calculated, using for each PWM a background score distribution which is derived by matching each PWM to the 85'603 unique phosphosites published in Johnson et al. 2023. 

```{r}
scores <- scorePhosphosites(pwms, sites$processed, scoreType = "percentile")
head(scores[,1:5])
```

The percentile rank is the matching score underlying the kinase activity predictions published in Johnson et al. Note that these percentile ranks do not account for phospho-priming, as they were missing in the background sites published in Johnson et al.
